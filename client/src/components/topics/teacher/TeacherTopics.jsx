...
export function TeacherTopics() {
      ...
      if (userEmail) {
        ...
        if (result.ok) {
          ...
          if (userData.length > 0) {
            ...
            // Pobierz teacher_id z userData i dodaj do formData
            if (teacherId) {
              setFormData((prevData) => ({ ...prevData, teacher_id: teacherId }));
            }
            ...
            // Pobierz klasy z danego szkoły
            const classAllFromSchoolQuery = `${backendServer}/classes/${schoolId}`;
            const classAllFromSchoolResult = await fetch(classAllFromSchoolQuery);
            const classAllFromSchoolData = await classAllFromSchoolResult.json();
            setClassData(classAllFromSchoolData);
            console.log("classAllFromSchoolData :", classAllFromSchoolData);
          } else {
            setError("Błąd pobierania danych użytkownika: brak danych.");
  ...
  const handleAddTopic = async () => {
    try {
      ...
      if (userData.length > 0) {
        ...
        // Sprawdź, czy teacher_id jest dostępne
        if (teacherId) {
          // Dodaj teacher_id do formData
          setFormData((prevData) => ({ ...prevData, teacher_id: teacherId }));
  
          // Dodaj log do sprawdzenia danych w konsoli
          console.log("Dane przed wysłaniem:", formData);
          ...
          const postData = {
            ...
            teacher_id: formData.teacher_id, // Dodaj teacher_id do danych wysyłanych
          };
  ...
  const fetchClassOptions = async () => {
    try {
      if (userData.length > 0) {
        const schoolId = userData[0].school_id;
        const classOptionsQuery = `${backendServer}/classes/${schoolId}`;
        console.log("classOptionsQuery", classOptionsQuery);
  ...
  const fetchSubjectOptions = async () => {
    try {
      if (userData.length > 0 && allClasses && allClasses.length > 0) {
        ...
        const schoolId = userData[0].school_id;
        const subjectOptionsQuery = `${backendServer}/subjects-for-class/${schoolId}/${classId}`;
        console.log("subjectOptionsQuery ", subjectOptionsQuery);
        const subjectOptionsResult = await fetch(subjectOptionsQuery);
  ...
  useEffect(() => {
    fetchClassOptions();
  }, [userData]); // Uruchomi się, gdy userData zostanie zaktualizowane
  
  useEffect(() => {
    if (formData.class_id) {
      fetchSubjectOptions(formData.class_id);
    }
  }, [formData.class_id]); // Uruchomi się, gdy formData.class_id zostanie zaktualizowane
  ...
  const handleChange = (e) => {
    ...
    console.log("handleChange - formData.class_id:", formData.class_id);
    console.log("handleChange - formData.subject_id:", formData.subject_id);
  };
  

  useEffect(() => {
    console.log("useEffect - formData.class_id:", formData.class_id);
  }, [formData.class_id]);
  
  useEffect(() => {
    console.log("useEffect - formData.subject_id:", formData.subject_id);
  }, [formData.subject_id]);
  ...
  const fetchData = async () => {
    try {
      ...
      // Tutaj możesz bezpiecznie korzystać z danych
    } catch (error) {
      console.error(error);
    }
  };
