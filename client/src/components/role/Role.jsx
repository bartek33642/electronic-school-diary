...
export function Role() {
  ...
  useEffect(() => {
    const fetchUserData = async () => {
      try {
        ...
        if (userEmail) {
          // Użyj adresu e-mail do zapytania o odpowiednie dane użytkownika
          ...
          if (result.ok) {
            // console.log("Dane pobrane z serwera:", userData);
  ...
  return (
    <div className="role-container">
      ...
      {userData.map((user) => (
        <Link to={`/${user.status}`} key={user.user_id}>
          <button className="btnBoxRole">
            <div className="boxRole">
              <span className="roleName">{user.status}</span>
