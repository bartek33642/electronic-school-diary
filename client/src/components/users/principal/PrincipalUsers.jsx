        ...
        if (result.ok) {
          ...
          if (userData.length > 0) {
            ...
            if (userPrincipalResult.ok) {
              const uniqueUserPrincipalIds = Array.from(
                new Set(userPrincipal.map((user) => user.school_id))
              );
              const uniqueUserPrincipal = userPrincipal.filter((user) =>
                uniqueUserPrincipalIds.includes(user.school_id)
              );
              setUserPrincipalData(uniqueUserPrincipal);
            } else {
              setError(
                "Błąd pobierania danych z danymi użytkowników - dla roli dyrektora."
              );
            }
          } else {
