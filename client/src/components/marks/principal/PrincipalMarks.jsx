...
export function PrincipalMarks() {
    ...
    const handleOpen = (gradeId) => {
      ...
      const addNewGrade = async () => {
        try {
            console.log("Dane do wysłania w metodzie POST (addNewGrade):", formData);
            if (!selectedStudentId || students.length === 0) {
            console.error("Błąd dodania oceny: Brak wybranego ucznia lub uczniów.");
            return;
        }
      ...
      useEffect(() => {
        const fetchTeachers = async () => {
            try {
                if (userData.length > 0) {
                    const schoolId = userData[0].school_id;
                    const teachersData = await fetch(`${backendServer}/all-teachers/${schoolId}`);
                    const teachers = await teachersData.json();
    ...
    return (
        <div className="principal-marks-container">
            <PrincipalMenu />
            <div className="principal-marks-elements">
                <h2>Oceny</h2>
...
>
 ...
 .map((mark) => (
   <div key={`${mark.grade_id}-${mark.student_id}`} className="grade-container">
      <div className="tooltip">
        <button
